...
fn bigtest() {
    ...
    let (mut compound_found, mut compound_ref) = mc_world.search_compound("ham", false);
    assert_eq!(compound_found, true);

    (compound_found, compound_ref) = mc_world.search_compound("hamm", false); //2 T, expected false
